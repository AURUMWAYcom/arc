...
uint256 hashGenesisBlock("0x1d3172d9601453007cc6b36b2f2de8a360618309af2151036129198c1e6e9c6a");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Aurumcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Aurumcoin Signed Message:\n";
...
{
    // Aurumcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Aurumcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Aurumcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Aurumcoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x1d3172d9601453007cc6b36b2f2de8a360618309af2151036129198c1e6e9c6a");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        block.nVersion = 1;
        block.nTime    = 1410882742;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084602107;
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("AurumcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("AurumcoinMiner : ProcessBlock, block not accepted");
    }
...
void static LitecoinMiner(CWallet *pwallet)
{
    ...
    try { loop {
        ...
        printf("Running Aurumcoin with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("AurumcoinMiner terminated\n");
        throw;
    }
}
